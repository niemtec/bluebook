...
We have all felt the relief of a program working and decided that a *_working_* mess is better than nothing. Leaving bad code behind to be cleaned up later only that *_later equals never_*.
...
- You should **avoid disinformation, that is to say leaving false clues which obscure the meaning of your code, even if an abbreviation looks good it might be disinformative.
   - Some names can also offer false clues, for example `accountList` shouldn’t be called that *_unless_* is is actually a `List`.
...
- **Methods should have a verb name since they are used to *_do_* things with. Accessors, mutators and predicates should be named for their values and prefixed with `get`, `set`, and `is`.
...
## Good Comments
Sometimes comments *can* be beneficial. These are known as *legal comments* and may include:
